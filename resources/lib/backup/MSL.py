...
class MSL(object):
	...
	def __tranform_to_dash(self, manifest):
		...
		subtitle_path = self.kodi_helper.get_setting('subtitle_folder')
		...
		for text_track in manifest.get('textTracks'):
			...
			for downloadable in text_track['downloadables']:
				...
				if	(filename_lang == self.kodi_helper.get_setting('subtitle_language')) or (self.kodi_helper.get_setting('subtitle_language') == ''):
					...
					if movie:
						filename_out = filename_title + '.' + filename_lang.upper() + '.vtt'
					else:
						filename_out = filename_title + '.S' + filename_season.zfill(2) + 'E' + filename_episode.zfill(2) + '.' + filename_lang.upper() + '.vtt' 
					...
					if self.kodi_helper.get_setting('convert_to_srt'):
